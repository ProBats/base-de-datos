...
select  		h.habitacion_numero habitaciones_disponibles,
h.piso,h.precio_por_noche,h.max_personas
from 		habitaciones h 
...
select hu.huesped_id, hu.nombres, hu.apellidos, inicio_fecha, fin_fecha, habitacion 
from huespedes hu 
join reservas r on hu.huesped_id = r.huesped;

-- 3. Obtener el total de reservas por huésped
select count(*) 'cantidad de reservas', huespedId 
from reservas r
join huespedes hu on r.huesped = hu.huespedId
group by huespedId;

-- 4. Listar las habitaciones ocupadas en una fecha específica
select h.habitacionNumero 
from habitaciones h 
join reservas r on h.habitacionNumero = r.habitacion
where '2025/06/06' 
between inicioFecha and finFecha;

-- 5. Obtener información de reservas junto con el precio total
select hu.huespedId, hu.nombres, hu.apellido,r.inicioFecha, r.finFecha, r.habitacion,hpiso,h.precioPorNoche,dateiff(r.inicioFecha,r.inicioFecha)*h.precioPorNoche
from reservas 
join huespedes hu on hu.huespedId = r.huesped
join habitaciones h on h.habitacionNumero = r.habitacion; 
...
update habitaciones set precio_por_noche = 55000 where habitacion_numero = 5;

-- 8. Eliminar una reserva
delete from reservas where reservas_id = 8;

-- 9. Contar el número total de habitaciones por piso
select piso, count(habitacion_numero) 'numero de habitaciones' 
from habitaciones 
group by piso;

-- 10.Listar todos los huéspedes que han reservado habitaciones 
-- en un rango de fechas
select hu.huesped_id,hu.nombres,hu.apellidos,r.inicio_fecha,fin_fecha,r.habitacion,h.piso
from reservas r 
join habitaciones h on h.habitacion_numero = r.habitacion
join huespedes hu on hu.huesped_id = r.huesped 
where inicio_fecha and fin_fecha between '2024/01/01' and '2025/03/30';

-- 11.Obtener habitaciones que permiten más de un cierto número de  personas
select habitacion_numero from habitaciones
where max_personas>=2;

-- 12.Listar huéspedes que han reservado una habitación con ducha
select distinct h.habitacion_numero,hu.nombres,hu.apellidos,h.tiene_ducha
from huespedes hu
join reservas r on hu.huesped_id=r.huesped
join habitaciones h on h.habitacion_numero=r.habitacion
where h.tiene_ducha=1
...
where precio_por_noche=(select max(precio_por_noche) from habitaciones);

-- 14.Obtener las reservas que terminan hoy
select * from reservas
where finFecha=curdate();

-- 15.Obtener el promedio de precio por noche de las habitaciones
select round(avg(precioPorNoche),2) precioPromedio from habitaciones;

-- 16.Mostrar habitaciones que tienen balcón
select habitacionNumero from habitaciones
where tieneBalcon=1;

-- 17.Buscar reservas de un huésped específico
select h.huespedId,h.nombres,h.apellidos from huespedes h
join reservas r on h.huespedId=r.huesped
where h.huespedId=2;

-- 18.Listar habitaciones y sus respectivos huéspedes
select h.habitacion_numero,hu.nombres,hu.apellidos from habitaciones h
join huespedes hu
on h.habitacionNumero=hu.huespedId
order by h.habitacionNumero;

-- 19.Contar el número de reservas por habitación
select h.habitacionNumero,count(r.habitacion) reservas 
from habitaciones h
join reservas r
on h.habitacionNumero=r.habitacion
group by r.habitacion;

-- 20.Eliminar huéspedes que no tienen reservas
delete 
from huespedes
where not in(select 1 from reservas where r.huesped=h.huespedId);